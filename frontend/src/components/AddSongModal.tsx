    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          {/* Parent ID */}
          <div>
            <label className="block text-sm font-medium text-slate-700">Parent Song ID (optional)</label>
            <input
              type="text"
              {...register('parentId')}
              className={clsx(
                'mt-1 block w-full rounded-lg border px-3 py-2 text-sm transition',
                errors.parentId ? 'border-red-500 ring-1 ring-red-500' : 'border-slate-300 focus:border-sky-400 focus:ring-1 focus:ring-sky-400',
              )}
              placeholder="Enter parent song UUID"
            />
            {errors.parentId && <p className="mt-1 text-sm text-red-500">{errors.parentId.message}</p>}
          </div>
