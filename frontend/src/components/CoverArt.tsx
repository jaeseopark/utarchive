...
export const CoverArt: FC<CoverArtProps> = ({
  ...
  const imageUrl = useMemo(() => (coverArtId ? getCoverArtUrl(coverArtId, size) : null), [coverArtId, size]);

  // Show placeholder if no cover art ID or image failed to load
  if (!imageUrl || hasError) {
    return (
      <div className="flex items-center justify-center" style={{ width: className ? 'inherit' : '100%', height: className ? 'inherit' : '100%' }}>
