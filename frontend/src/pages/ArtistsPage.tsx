...
function ArtistsPage() {
  ...
  useEffect(() => {
    ...
    api
      .get(`/api/artists?limit=${PAGE_SIZE}&offset=${page * PAGE_SIZE}`, ArtistsResponseSchema)
      .then(setArtists)
  ...
  return (
    <section className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-slate-400">Browse all artists in the archive.</p>
      </div>

      <div className="overflow-x-auto rounded-3xl border border-slate-800 bg-slate-950/80 p-4 shadow-xl shadow-slate-950/20">
        {isLoading ? (
          <div className="min-h-[240px] flex items-center justify-center text-slate-400">Loading artists…</div>
        ) : error ? (
          <div className="min-h-[240px] rounded-2xl border border-rose-600 bg-rose-950/30 p-4 text-rose-100">Error loading artists: {error}</div>
        ) : rows.length === 0 ? (
          <div className="min-h-[240px] flex items-center justify-center text-slate-400">No artists found.</div>
        ) : (
              ...
              {rows.map((artist) => (
                <tr key={artist.id} className="border-b border-slate-800 last:border-b-0">
                  <td className="px-4 py-4">
                    <Link to={`/artists/${artist.id}`} className="font-medium text-slate-100 transition hover:text-sky-300">
                      {artist.name}
                    </Link>
                  </td>
                  <td className="px-4 py-4 text-slate-300">{artist.aliasText}</td>
                  <td className="px-4 py-4 text-slate-300">{artist.songCount}</td>
                </tr>
              ))}
            </tbody>
          </table>
